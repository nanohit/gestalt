...
import { cloneSiteContent, defaultContent, SiteContent } from "../lib/content";
...
export function createContentManager(endpoint: string, initialContent: SiteContent = defaultContent) {
  return function useContentManager(isAdmin: boolean) {
    const [content, setContent] = useState<SiteContent>(() => cloneSiteContent(initialContent));
...
const useContentInternal = createContentManager("/api/content");

export function useSiteContent(isAdmin: boolean) {
  return useContentInternal(isAdmin);
}
